...
describe('EngagementQueue (T120)', () => {
  ...
  describe('mount / unmount', () => {
    it('mount creates a single root element inside the container', () => {
      const q = newQueue();
      q.mount(container);
      const root = container.querySelector('.replymate-queue');
      expect(root).not.toBeNull();
      expect(container.querySelectorAll('.replymate-queue')).toHaveLength(1);
    });
    ...
    it('unmount removes the root element', () => {
      ...
      q.mount(container);
      q.unmount();
      expect(container.querySelector('.replymate-queue')).toBeNull();
    });
  ...
  describe('refresh — rendering', () => {
    ...
    it('each tile shows score, author name, and one of the why-reasons', async () => {
      ...
      const tile = container.querySelector('.replymate-queue__tile');
  ...
  describe('tile actions', () => {
    ...
    it('clicking hide invokes dismiss and removes the tile from the DOM', async () => {
      ...
      const before = container.querySelectorAll('.replymate-queue__tile').length;
      ...
      const after = container.querySelectorAll('.replymate-queue__tile').length;
  ...
  describe('tone / length controls', () => {
    it('changing tone triggers a regenerate for all visible tiles', async () => {
      ...
      const toneSelect = container.querySelector('.replymate-queue__tone') as HTMLSelectElement;
    ...
    it('changing length triggers a regenerate and propagates the new length', async () => {
      ...
      q.mount(container);
      await q.refresh([makeParsedPost()]);
      const lengthSelect = container.querySelector('.replymate-queue__length') as HTMLSelectElement;
