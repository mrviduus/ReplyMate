...
**Migration:** `migrateIfNeeded()` runs on background startup; bumps `replymate.schema.version`.
...
|--------------------------------------------|-------------------------|----------------------------------------------------------|--------------------------------|
| `replymate.profile.v1`                     | `ProfileContext`        | Captured user profile + positioning summary              | Manual; stale chip after 30d   |
| `replymate.queue.engaged.v1`               | `EngagedPost[]`         | Posts the user has copied a draft for                    | TTL filter (30d) on read       |
| `replymate.queue.dismissed.v1`             | `string[]`              | Post IDs the user explicitly hid                         | Forever (until manual clear)   |
| `replymate.queue.preferences.v1`           | `QueuePreferences`      | defaultTone, defaultLength, autoRefreshMinutes, sidebar  | Forever (write deferred)       |
| `replymate.ssi.history.v1`                 | `SsiSnapshot[]`         | Daily SSI snapshots                                      | FIFO, cap 90                   |
| `replymate.ssi.lastError.v1`               | `SsiLastError`          | Last parse/capture failure for popup chip                | Cleared on next success        |
| `replymate.connections.suggestions.v1`     | `ConnectionSuggestion[]`| Today's connection suggestions (v0.4.1)                  | Daily refresh, 7d history      |
| `replymate.connections.draftedThisWeek.v1` | `number`                | Counter for safe-limit throttle (v0.4.1)                 | Resets Monday 00:00 UTC        |
| `replymate.schema.version`                 | `number`                | Migration anchor                                         | Bump on breaking change        |
...
### EngagedPost
Persisted under `replymate.queue.engaged.v1`. `getEngagedPosts()` filters expired in-memory.
```typescript
{
  postId: string;
  engagedAt: number;
...
### SsiSnapshot
Persisted under `replymate.ssi.history.v1`.
```typescript
{
  total: number;                  // 0..100
  components: {
...
### SsiLastError
Persisted under `replymate.ssi.lastError.v1`; surfaces as popup chip until next success.
```typescript
{
  message: string;
  capturedAt: number;
...
Persisted under `replymate.connections.suggestions.v1`.
```typescript
{
  profileUrl: string;
  name: string;
...
### QueuePreferences
Persisted under `replymate.queue.preferences.v1`. `sidebarPosition` write currently deferred.
```typescript
{
  defaultTone: ToneKey;
