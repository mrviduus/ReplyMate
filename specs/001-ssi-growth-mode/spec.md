...
The user opens the ReplyMate popup and sees a new "SSI" tab. The tab displays the most recent SSI snapshot (total + 4 component scores), a 30-day trend graph, and an actionable insight ("Your 'Engage with insights' score dropped from 0.85 to 0.74 over the past 7 days. Open the Engagement Queue to recover."). A daily background job captures fresh SSI data automatically; the user may also tap "Refresh now" for an on-demand capture.
...
1. **Given** the user has just installed ReplyMate and is on their own LinkedIn profile page, **When** the user clicks "Capture profile" in popup, **Then** within 30 seconds the popup shows the captured fields and a positioning summary.
...
- **FR-020**: System MUST schedule a daily `chrome.alarms` job ("replymate.ssi.daily", periodInMinutes: 1440) on install.
...
- **SC-006**: Zero LinkedIn account warnings, restrictions, or bot-detection challenges attributed to ReplyMate over a 90-day dogfood window.
...
- ReplyMate Constitution: `.specify/memory/constitution.md` (v1.1.0+ required — amended 2026-05-14 with carve-outs, MV3 lifecycle, fixture hygiene)
