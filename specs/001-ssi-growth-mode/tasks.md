---
description: "Task list for SSI Growth Mode (ReplyMate v0.4.0) — v1.1"
---
...
- [ ] **T034** [US3] Implement `src/profile-injected.ts` — the script that gets injected. Contains ONLY the parser invocation (`parseProfileDom(document)`) and returns the result. No long-lived listeners, no message passing, no DOM mutation. This file is bundled but NOT listed in `content_scripts`; Parcel `web_accessible_resources` exposes its compiled JS path for `executeScript`.
- [ ] **T035** [US3] Implement `src/profile-context.ts` exporting `ProfileContextService` (popup-side) with `capture()`, `get()`, `shouldRefresh()`. `capture()` flow:
  ...
  Make T033 pass.
- [ ] **T036** [US3] Wire `profile.capture` action handler in `src/background.ts`: receives RawProfileFields, builds prompt via `prompt-builder.buildPositioningPrompt`, runs WebLLM via existing inference path, returns positioningSummary. Background does NOT touch tabs or scripting for this flow.
- [ ] **T037** [US3] Add "Profile" tab to `popup.html` with: "Capture profile" button, active-tab URL hint ("Currently on: …"), captured fields display, "Refresh" button (only enabled when on `/in/`), captured-at timestamp, optional "Open my profile ↗" CTA when not on a profile page. Update `popup.ts` to wire to `ProfileContextService.capture()` and `get()`.
- [ ] **T038** [US3] Update `src/popup.css` for new Profile tab styles. Match existing aesthetic.
- [ ] **T039** [US3] Update `src/manifest.json`: add `"scripting"` to `permissions`. Add `profile-injected.js` to `web_accessible_resources` with match `https://www.linkedin.com/in/*`. Confirm NO entry added to `content_scripts` for `/in/*`.
