...
export class EngagementQueue {
  ...
  mount(container: Element): void {
    ...
    root.className = 'replymate-queue';
    root.setAttribute('role', 'complementary');
    root.setAttribute('aria-label', 'ReplyMate Engagement Queue');
    root.innerHTML = `
      <header class="replymate-queue__header">
        <span class="replymate-queue__title">ReplyMate · Engagement Queue</span>
        <button class="replymate-queue__close" type="button" aria-label="Close queue">×</button>
      </header>
      <div class="replymate-queue__controls">
        <label class="replymate-queue__control">
          <span>Tone</span>
          <select class="replymate-queue__tone">
        ...
        <label class="replymate-queue__control">
          <span>Length</span>
          <select class="replymate-queue__length">
      ...
      <div class="replymate-queue__list" role="list"></div>
    `;
    container.appendChild(root);

    this.root = root;
    this.toneSelect = root.querySelector('.replymate-queue__tone') as HTMLSelectElement;
    this.lengthSelect = root.querySelector('.replymate-queue__length') as HTMLSelectElement;
    this.listEl = root.querySelector('.replymate-queue__list') as HTMLElement;
    ...
    root.querySelector('.replymate-queue__close')?.addEventListener('click', () => this.unmount());
    ...
    this.listEl.addEventListener('click', (ev) => {
      ...
      if (!btn) return;
      const tile = btn.closest('.replymate-queue__tile') as HTMLElement | null;
  ...
  private renderTile(s: ScoredPost, rank: number): string {
    ...
    return `
      <article class="replymate-queue__tile" data-post-id="${esc(s.id)}" role="listitem">
        <header class="replymate-queue__tile-header">
          <span class="replymate-queue__rank">#${rank}</span>
          <span class="replymate-queue__score" data-category="${s.relevance.category}">${s.relevance.score}</span>
          <span class="replymate-queue__category">${esc(s.relevance.category)}</span>
        </header>
        <div class="replymate-queue__tile-author">
          ${esc(s.authorName || 'Unknown')}${s.authorTitle ? ` · ${esc(s.authorTitle)}` : ''}
        </div>
        <div class="replymate-queue__tile-why">Why: ${esc(reason)}</div>
        <textarea class="replymate-queue__draft" aria-label="Editable draft reply">Drafting…</textarea>
        <div class="replymate-queue__tile-actions">
          <button type="button" data-action="regenerate">↻ Regenerate</button>
          <button type="button" data-action="copy" class="replymate-queue__copy">📋 Copy &amp; Open</button>
          <button type="button" data-action="hide" class="replymate-queue__hide">Hide</button>
        </div>
      </article>
    `;
  }
